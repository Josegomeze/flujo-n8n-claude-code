# Instrucciones para Cowork — Comparar el sistema web vs el Cotizador HTML v81

**Objetivo:** verificar que el sistema web (`zonasegura.financieralaprosperidad.app`) y el
Cotizador HTML v81 producen **los mismos resultados**, copiando 10 cotizaciones reales
(lo más dispares posible) del sistema web y reproduciéndolas en el v81.

---

## Preparación (antes de pegar el prompt)

1. Adjunta a la sesión de Cowork el archivo **`Cotizador_Fiflouu__web__v81.html`**.
...
2. TODOS los cálculos se hacen ÚNICAMENTE en el archivo adjunto
   Cotizador_Fiflouu__web__v81.html (ábrelo en el navegador).
3. En el v81 verifica antes de cotizar: el botón de método debe decir
...
4. No modifiques parámetros de la financiera en el v81. Si en el PASO 1 encuentras
...
b) En el v81 abre "⚙ Ver / editar Parámetros financiera" y la pestaña
   "Tablas (ref)" de la sección avanzada.
c) Compara y muestra una tabla de verificación (parámetro | web | v81 | ¿igual?):
...
PASO 3 — REPRODUCIR CADA CASO EN EL v81 (aquí SÍ calculas):
Para cada uno de los 10 casos, en el HTML v81:
   ...
   e) Lee los resultados de la cotización del v81.

PASO 4 — TABLA COMPARATIVA FINAL:
Muestra una tabla con TODOS los casos y conceptos:
   Caso | Concepto | Sistema web | HTML v81 | Diferencia | ✓/✗
Criterios:
...
Tras la comparación de 10 cotizaciones (01/07/2026), el v81 ya corrige: meses de
interés por cronograma real (efecto diciembre) y servicio 3% en Débito y
Voluntario. Quedan 2 puntos por resolver con datos del web:

1. **Notaría**: el web varía ($24.71–$25.64) y el v81 usa $25 fijo. Pedir a Cowork
   que averigüe la fórmula (¿aparece en el detalle de la cotización o en alguna
   pantalla de configuración?).
2. **Comisión promotor en Voluntario y Platinum a plazos largos**: pedir a Cowork
   los valores exactos (celda por celda, con encabezados) de las columnas
   "comision" y "costo" de tabla_matrices por producto, y 2-3 cotizaciones más de
   Voluntario y de Platinum ≥60 cuotas para triangular la regla.

## Notas para ti (no van en el prompt)

- **Regla CSS confirmada:** Pago Automático CSS tope de plazo = **48 meses** (decisión
  de negocio). El v81 la aplica en el tope, las cuotas y la tabla de umbral. El sistema
  web usa un bucket "Débito" de 72 sin separar CSS: las cotizaciones CSS del web con
  plazo > 48 diferirán del v81 **a propósito** (el web es el que debería corregirse).

- **Por qué ITBMS financiado + interés agregado:** son los valores por defecto del v81
...
- Complemento opcional: en el repositorio está `quote-engine/golden/casos_dorados.csv`
  (1,095 casos entrada→salida generados del v81 con un cliente de prueba fijo). Sirve
