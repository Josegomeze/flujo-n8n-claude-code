# Instrucciones para Cowork — Comparar el sistema web vs el Cotizador HTML v82

**Objetivo:** verificar que el sistema web (`zonasegura.financieralaprosperidad.app`) y el
Cotizador HTML v82 producen **los mismos resultados**, copiando 10 cotizaciones reales
(lo más dispares posible) del sistema web y reproduciéndolas en el v82.

---

## Preparación (antes de pegar el prompt)

1. Adjunta a la sesión de Cowork el archivo **`Cotizador_Fiflouu__web__v82.html`**.
...
2. TODOS los cálculos se hacen ÚNICAMENTE en el archivo adjunto
   Cotizador_Fiflouu__web__v82.html (ábrelo en el navegador).
3. En el v82 verifica antes de cotizar: el botón de método debe decir
...
4. No modifiques parámetros de la financiera en el v82. Si en el PASO 1 encuentras
...
b) En el v82 abre "⚙ Ver / editar Parámetros financiera" y la pestaña
   "Tablas (ref)" de la sección avanzada.
c) Compara y muestra una tabla de verificación (parámetro | web | v82 | ¿igual?):
...
PASO 3 — REPRODUCIR CADA CASO EN EL v82 (aquí SÍ calculas):
Para cada uno de los 10 casos, en el HTML v82:
   ...
   e) Lee los resultados de la cotización del v82.

PASO 4 — TABLA COMPARATIVA FINAL:
Muestra una tabla con TODOS los casos y conceptos:
   Caso | Concepto | Sistema web | HTML v82 | Diferencia | ✓/✗
Criterios:
...
El v82 ya corrige (validado contra los reportes de cotizaciones reales):
servicio 3% en todas las claves, meses de financiamiento CALENDARIO (incluye el
mes inicial parcial y los diciembres; 7/7 casos del portal exactos) y el plazo en
Débito/Pago Automático (respeta el solicitado, topado a 72/48).

Quedan por verificar con datos del web:
1. **Interés**: re-verificar los casos; con los meses calendario ya igualados
   debería quedar exacto al centavo.
2. **Notaría**: el portal varía ($24.71–$25.64) y el v82 usa $25 fijo. Averiguar
   la fórmula (detalle de la cotización o pantalla de configuración, solo ver).
3. **Débito con cliente real**: reproducir C2/C5 con el salario real del cliente
   del portal (o cotizarlas por letra) para compararlas de verdad.

## Notas para ti (no van en el prompt)

- **Regla CSS confirmada:** Pago Automático CSS tope de plazo = **48 meses** (decisión
  de negocio). El v82 la aplica en el tope, las cuotas y la tabla de umbral. El sistema
  web usa un bucket "Débito" de 72 sin separar CSS: las cotizaciones CSS del web con
  plazo > 48 diferirán del v82 **a propósito** (el web es el que debería corregirse).

- **Por qué ITBMS financiado + interés agregado:** son los valores por defecto del v82
...
- Complemento opcional: en el repositorio está `quote-engine/golden/casos_dorados.csv`
  (1,095 casos entrada→salida generados del v82 con un cliente de prueba fijo). Sirve
