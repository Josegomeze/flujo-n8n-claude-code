...
// {entradas -> salidas} para AMBOS caminos (por monto y por letra) + la configuración.
...
function caseExpr(ced,tipo,prom,mode,valor,plazo){
  const setMonto = mode==='monto' ? `setRaw('Calculadora','D8',0); setRaw('Calculadora','D2', ${valor});`
                                  : `setRaw('Calculadora','D2',0); setRaw('Calculadora','D8', ${valor});`;
  return `(()=>{
    ...
    return {out, promotorNoTiene:(String(b6)==='No tiene')};
  })()`;
}
...
try{
  ...
  const letras=[50,100,200,400,700];
  const plazos=[18,24,36,48,60];
  for(let tipo=1; tipo<=7; tipo++) for(let prom=1; prom<=5; prom++) for(const plazo of plazos){
    for(const monto of montos){
      const rec=await ev(send, caseExpr(ced,tipo,prom,'monto',monto,plazo));
      cases.push({mode:'monto', in:{monto,plazo,tipoCode:tipo,promotorIdx:prom,clave:'G8',esCSS:false,promotorNoTiene:rec.promotorNoTiene}, out:rec.out});
    }
    for(const letra of letras){
      const rec=await ev(send, caseExpr(ced,tipo,prom,'letra',letra,plazo));
      cases.push({mode:'letra', in:{letra,plazo,tipoCode:tipo,promotorIdx:prom,clave:'G8',esCSS:false,promotorNoTiene:rec.promotorNoTiene}, out:rec.out});
    }
  }
  writeFileSync(OUT, JSON.stringify({meta:{ced,source:'sheet v76',n:cases.length}, config, cases}));
  const withErr=cases.filter(c=>Object.values(c.out).some(v=>typeof v==='string'&&String(v).startsWith('ERR:')));
  console.log('Oráculo:', cases.length, 'casos (monto+letra) ->', OUT, '| con error:', withErr.length);
  ws.close();
